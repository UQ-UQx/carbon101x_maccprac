<?php
    ...
    //Dont use the following - experimenting (ankith)

    // function setLeaderboardStatus($lti_id, $points){
    //     global $lti;
    //     $db = Db::instance();
    //     date_default_timezone_set('Australia/Brisbane');
    //     $modified = date('Y-m-d H:i:s');
    //     $existing = getLeaderboardStatus($lti_id);
    //    // error_log($lti->user_id());
    //     if(!$existing) {
    //         $db->create('leaderboardstatus', array('lti_id'=>$lti_id,'user_id'=>$lti->user_id(), 'points'=>$points,'created'=>$modified,'updated'=>$modified));
    //     } else {
    //        // $db->update('states', array('state'=>$state,'updated'=>$modified), 'lti_id', $lti_id);
    //        // 
    //        error_log("setting leaderboard status".json_encode($existing),0);
    ...
    // function getLeaderboardStatus($lti_id){
    //     global $lti;

    //     $db = Db::instance();
    //     $select = $db->query( 'SELECT points FROM leaderboardstatus WHERE lti_id = :lti_id AND user_id = :user_id', array( 'lti_id' => $lti_id, 'user_id' => $lti->user_id() ) );
    //     while ( $row = $select->fetch() ) {
    //         return $row;
    //     }
    //     return null;
